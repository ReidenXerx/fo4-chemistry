 ...
 *
 * It checks the BYTES of the archive it is about to upload rather than trusting
 * the packaging script: the plugin and the compiled scripts must both be in
 * there and be a plausible size. Rapport's equivalent refuses to ship
 * diagnostics; Chemistry has no config to get wrong, so this guards against the
 * other classic -- publishing an archive built from a stale or empty tree.
 *
...
const NEXUS = { gameDomain: "fallout4", modId: 109225, title: "Chemistry" };
...
const zipName = `Chemistry-${version}.zip`;
...
// Chemistry ships a plugin and compiled scripts and nothing else -- there is no
// config here to leak a developer's switch, which is the whole reason the
// policy lives in one mod and the framework in another. So the gate checks the
// two things an empty or stale build would get wrong.
const esp = zipEntry(bytes, "Chemistry.esp");
if (!esp) fail("Chemistry.esp is not in the archive");
if (esp.length < 200) fail(`Chemistry.esp is only ${esp.length} bytes — that is not a built plugin`);
info(`Chemistry.esp: ${esp.length} bytes`);

const pex = zipEntry(bytes, "Scripts/Chemistry/Autonomy.pex");
if (!pex) fail("Scripts/Chemistry/Autonomy.pex is not in the archive — run build-papyrus first");
info(`Autonomy.pex: ${pex.length} bytes`);
...
const client = nexusClient({ apiKey: readApiKey(), userAgent: `ChemistryRelease/${version}` });
