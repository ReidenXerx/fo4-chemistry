...
Chemistry needs no assets of its own, no textures, no animations and no MCM. Its tunables are
`AutoReadOnly` properties in the script, because Chemistry has no native code with which to read an
ini.
